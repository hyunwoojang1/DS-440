"""EntryScoreEngine — full pipeline orchestrator."""
...
class EntryScoreEngine:
    ...
    LOOKBACK_TECH_DAYS   = 1500   # 40 monthly bars + buffer
    ...
    def run(
        ...
        # Full market fundamental data (includes gsector, tic) — sector z-score population
        ...
        # Ticker fundamental values + GICS sector code
        ticker_fund_raw, sector_code = self._wrds_fetcher.get_ticker_fundamentals(
            ticker, all_fund_df, as_of
        )

        # Sector latest value distribution (z-score denominator)
        ...
        # Sector z-score based FundamentalScorer
...
def _latest_values(df: pl.DataFrame, as_of: str) -> dict[str, float]:
    """Extracts per-column float values from the most recent row before as_of."""
...
def _dict_to_pl_df(values: dict[str, float], as_of: str) -> pl.DataFrame:
    """Converts a single snapshot dictionary to a Polars DataFrame."""
